...
prelim.data$Experiment.ID <- prelim.data$Experiment.ID %>% factor 
...
Here we show the residual effect sizes for each outcome measured. The following forest plot is based on the residual effect sizes. The model is an intercept only model with the same multilevel random effects structure as we use later when investigating environment and sex
...
#Create model we later establish and analyse but not restricted
forest.model <- rma.mv(g, var.g, mods = ~ 1, random = ~ 1 | Study.ID/Outcome, method = "REML", data = prelim.data)
...
> This inner, outer factor stuff from the metafor package is a bit strange. There is a goodish description in ``?rma.mv()`` but still unsure how it differs from the above model. It seems that it is useful to breakdown variance-covariance matrix but unsure how that would benefit our analysis.
...
varSF <- rma.mv(lnCVr, var.lnCVr, data = prelim.data2, mods = ~ 1, random = ~ 1 | Study.ID, subset = (Environment == "Stressed" & Sex == "F"))

# For stressed environment and females
varSM <- rma.mv(lnCVr, var.lnCVr, data = prelim.data2, mods = ~ 1, random = ~ 1 | Study.ID, subset = (Environment == "Stressed" & Sex == "M"))

# For stressed environment and females
varSB <- rma.mv(lnCVr, var.lnCVr, data = prelim.data2, mods = ~ 1, random = ~ 1 | Study.ID, subset = (Environment == "Stressed" & Sex == "B"))

#For Benign environment and females
varUF<- rma.mv(lnCVr, var.lnCVr, data = prelim.data2, mods = ~1, random = ~ 1 | Study.ID, subset = (Environment == "Unstressed" & Sex == "F"))

#For Benign environment and males
varUM<- rma.mv(lnCVr, var.lnCVr, data = prelim.data2, mods = ~1, random = ~ 1 | Study.ID, subset = (Environment == "Unstressed" & Sex == "M"))

#For Benign environment and both
varUB <- rma.mv(lnCVr, var.lnCVr, data = prelim.data2, mods = ~1, random = ~ 1 | Study.ID, subset = (Environment == "Unstressed" & Sex == "B"))
...
```{r}
funnel.rma(model.Ambiguous)
regtest(model.Ambiguous, predictor = "vi")
funnel.rma(model.Indirect)
regtest(model.Indirect)
funnel.rma(model.Direct)
regtest(model.Direct)
funnel.rma(model.complete2)
```
...
```{r}
prelim.data %>% ggplot(aes(x=JIF, y=(g), size = 1/sqrt(var.g)))+
  geom_jitter()+
...
```{r}
prelim.data %>% ggplot(aes(x=Year, y=g, size = (1/(var.g))))+
  geom_jitter()+
  geom_hline(yintercept=0, linetype = 'dotted')+
  geom_smooth(method='lm')+
  labs(size = 'weight')
...
```{r}
df.forest.model %>% ggplot(aes(x=Blinding, y=g, color = Outcome.Class))+
  geom_boxplot()+
  geom_hline(yintercept=0, linetype = 'dotted')
```
