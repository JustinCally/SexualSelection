...
Firstly, althought the effect size prediction for the trait has 95 % CIs not overlapping zero, the overall magnitude of the effect size is small (see Table exerpt), and the Bayes Factor (an estimate of the likelihood that the effect size is actually positive) is also small. The full table for all predicted effect sizes used in Figure 1 can be seen in **S8**. Below we provide the exerpt of this table and forest plot for "Male attractiveness".  

Secondly, the effect size predictions were made using a single model that incorperated all 459 effect sizes, thus model predictions for each trait have narrower confidence intervals as the fitted values do not incorperate measurement error. In regards to our analysis, this means that the predictions that appear in Figure 1 are sourced from using ``fitted.brmsfit`` as opposed to ``predict.brmsfit``. Given the reviewers comments we have clarified how we obtained model predictions used in our results. Additionally, some meta-analyses investigating the effect of a variable on more than one outcome model each outcome (in this case the *fitness component*) seperately. Arguably, this method provides more variable model estimates as models will always have lower power due to reduced sample sizes and the incorperation of measurement error in the predictions. We have added a table to the Supplementary Material where models are independently conducted on all fitness components where n > 3 (**Table S9**).  
...
**Add section in discussion about the variability amongst traits and why some might be subject to trade offs (e.g. immunity).**

**Summarise results for direct, indirect and ambiguous**

The reviewer provides an accurate shortcoming of discussion on the observed heterogeneity between fitness components. We have extended certain parts of the discussion related to this aspect...

In regards to conducting a meta-analysis on just direct fitnesss components, the reviewer provids a valid question of our use of direct **and** indirect fitness components for the meta-analysis investigating the effects of sex and stress. To some degree the use of direct **and** indirect is arbitrary. But based on the question we asked we were interested in the effect of sexual selection on many components of fitness. 

Nevertheless, we conducted our analysis using just direct fitness measures. We find the same key associations; that sexual selection generally elevates fitness, especially for females, with the effect magnified in stressful environments. Below we present the findings, which we have incorperated into our supplementary material and referenced within the manuscript and Figure 2 caption. 
...
```{r, fig.height=4.5, fig.width=6.25, warning=FALSE, message=FALSE, error = FALSE, fig.align='center'}
...
post <- (posterior_samples(brms.direct, 
...
```{r, warning=FALSE}
direct.only.data <- strict_dataset %>% filter(Outcome.Class == "Direct")
I2(model.direct.only, direct.only.data$var.g) %>% pander(digits = 3)
```
...
No difference... kind of there for documentation on how we created it (transparency). What do you think we should do about this Luke?
...
Reviewer 2 has rightly pointed out that the use of Hedge's *g* has been critisized for its ability to account for heterogeneity between the control and treatment groups **CITE**. We extended our meta-analysis by including the log response ratio (lnRR) as well as a modified version of Hedge's g that attempts to account for this heterogeneity, this modified statistic (**CITE**) can be derived from the ``metafor`` package for meta-analysis, where it is named SMDH (standardized mean difference with heteroscedastic population variances in the two groups). There are several key findings from the use of lnRR and SMDH: 

*Firstly*, Hedge's g and SMDH are highly correlated. This suggests that unequal variance between the treatment and control groups (heteroscedasticity) is unlikely to impact the effect size of Hedge's g
...
```{r, warning = F, fig.width=7, fig.height = 3.5, message = F}
grid.arrange(both.plots + 
               guides(fill = FALSE) + 
               ggtitle("Comparison between Hedge's g and lnRR effect sizes \n\n"), 
             lnRR.plots + ggtitle("\n\n"), nrow = 1)
```

For these reasons we have decided to use Hedge's g as the main effect size throughout the manuscript but reference and provide equivelant statistical analysis for lnRR in the supplementary material.  
...
The reviewer suggestions have been included in our meta-analysis in order to validate our conclusions. Several key points arise from the meta-analysis of variation: 

*Firstly*, as the reviewer suggested we justify the use of lnCVR my assessing the mean-variance relationship. Given this finding the use of lnCVR is preferenced over lnVR, as lnVR does not account for the mean-variance relationship.  
...
*Secondly*, We conducted both Bayesian and REML meta-analyses for lnVR, which we present in the supplementary material and briefly discuss in the manuscript. We found similar trends for the meta-analysis of variance using lnCVR and the meta-analysis of variance using lnVR, These are: 
...
+  Bayesian models suggest that under stressful conditions elevated sexual selection causes a significant decrease in the variance. But this trend is not significant for REML models. 

However, when using lnVR, the variability of fitness traits for males under sexual selection increased, while in the model using lnCVR it did not. 


```{r, warning = F, fig.width=7, fig.height = 3.5, message = F}
grid.arrange(both.var.plots + 
               guides(fill = FALSE) + 
               ggtitle("Comparison between lnCVR and lnVR effect sizes \n\n"), 
...
Our mstudy does not involve a systematic review.
