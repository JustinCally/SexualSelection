...
<input type=button class=hideshow></input>
```{r}
Outcome_and_sex <- table(prelim.data$Outcome, prelim.data$Sex) %>%
  pander(split.cell = 40, split.table = Inf)
```
...
ggsave(plot = Taxon.metaanlysis, filename = "figures/Taxon_metaanalysis_plot.eps", height = 5, width = 7.5)
Taxon.metaanlysis
```
...
ggsave(plot = EnvSex.metaanalysis, filename = "figures/Environment_Sex_metaanalysis_plot.eps", height = 5, width = 7.5)
EnvSex.metaanalysis
```
...
ggsave(plot = var.plot, filename = "figures/Variance_plot.eps", height = 5, width = 7.5)
var.plot
```
...
#make.funnel(restricted.data, model.complete2)
```
