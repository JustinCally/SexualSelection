...
#Have taxon as fixed effects
brms.complete <- brm(g | se(SE)  ~ 1 + Sex * Environment + Taxon
                + (1|Study.ID) #group level effect #1
                + (1|Outcome), #group level effect #2 
                family = "gaussian", 
                seed = 1,
                cores = 4, chains = 4, iter = 4000, # Run 4 chains in parallel for 4000 iterations (2000 are burn in)
                control = list(adapt_delta = 0.999, max_treedepth = 15),
...
bayesplot::mcmc_areas(posterior_samples(brms.complete)[,1:13]) +
  geom_vline(xintercept = 0, linetype = 2)
...
meta.brms <- brm(g| se((SE), sigma = TRUE)  ~ 1 + Sex * Environment #response is Hedges' g and the standard error associated with it (SE), sex, environment and their interaction are moderators
                + (1|Study.ID) #group level effect #1
...
#You can obtain a posterior sampling through the ``fitted.brmsfit`` function, which gives the same posterior values as doing it manually (see below).**Note that the fitted values obtained here have much smaller error than the predict values, which blow out**
...
```{r}
post <- (posterior_samples(meta.brms, 
...
posterior.plot <- post %>% ggplot()+
  stat_density_ridges(aes(x=value, y = Environment, fill = Sex), alpha = 0.4, scale = 0.6, position = position_nudge(y = 0.15), height = 10, show.legend = F, quantile_lines = T, quantiles = 2)+
  geom_vline(xintercept = 0, linetype = 2, colour = "black") + 
    ylab("Environment")+
  xlab("\nEffect Size (Hedges' g)")+
  scale_fill_brewer(palette = "Set1")+
...
```
<br></br>
**Figure X** Posterior fitted values for Hedges' g alongside the predictions from metafor
...
restricted.data$var.lnCVr <- Calc.var.lnCVR(restricted.data$mean.low, restricted.data$sd.low, restricted.data$n.low, restricted.data$mean.high, restricted.data$sd.high, restricted.data$n.high, Equal.E.C.Corr=F)


restricted.data <- restricted.data %>% drop_na(lnCVr)
```
...
variance.model <- rma.mv(lnCVr, var.lnCVr, mods = ~ 1 + Sex*Environment, 
                          random = list(~ 1 | Study.ID, 
                                       ~ 1 | Outcome), 
                         method = "REML", data = restricted.data)
summary(variance.model)

#Now with brms

#Use brm to create a model similar to the one used in the REML approach. 
variance.brms <- brm(lnCVr| se((SE.v), sigma = TRUE)  ~ 1 + Sex * Environment #response is Hedges' g and the standard error associated with it (SE), sex, environment and their interaction are moderators
                + (1|Study.ID) #group level effect #1
                + (1|Outcome), #group level effect #2 (unrelated/crossed) with #1
                family = "gaussian", #default
                seed = 1,
                cores = 4, chains = 4, iter = 4000, #Run 4 chains in parrallel for 4000 iterations (2000 are burn in)
                control = list(adapt_delta = 0.999, max_treedepth = 15),
                data = restricted.data %>% mutate(SE.v = sqrt(var.lnCVr)))

summary(variance.brms)

post.variance <- (posterior_samples(variance.brms, 
                           pars = c("b_Intercept", "b_SexB", "b_SexF", 
                                    "b_EnvironmentStressed", "b_SexB:EnvironmentStressed", 
                                    "b_SexF:EnvironmentStressed")) %>%
         mutate(both_benign = b_Intercept + b_SexB,
...
```{r, fig.height= 5.5, fig.width=5.5, warning = FALSE}
...
var.plot.posterior <- post.variance %>% ggplot()+
  stat_density_ridges(aes(x=value, y = Environment, fill = Sex), alpha = 0.4, scale = 0.6, position = position_nudge(y = 0.15), height = 10, show.legend = F, quantile_lines = T, quantiles = 2)+
  geom_vline(xintercept = 0, linetype = 2, colour = "black") + 
  ylab("Environment\n")+
...
# pdf("figures/both.var.plots.pdf",  width = 5.5, height = 5.5)
# both.var.plots
# dev.off()
```
<br></br>
...
```{r, fig.height= 7.5, fig.width=10, warning=FALSE}
generations.plot.var <- restricted.data %>% ggplot(aes(x=Generations, y=lnCVr))+
  geom_point(shape=21, color = "grey20", size=2, aes(fill=Taxon))+
  ylim(-3.5,3.5)+
  geom_hline(yintercept=0, linetype="dashed") + 
  scale_fill_brewer(palette = "Set3")+
