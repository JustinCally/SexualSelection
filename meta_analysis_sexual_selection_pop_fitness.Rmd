...
#Use brm to create a model similar to the one used in the REML approach. 
meta.brms <- brm(g| se(sqrt(var.g), sigma = TRUE)  ~ 1 + Sex * Environment #response is Hedges' g and the standard error associated with it (sqrt(var.g)) with sex, environment and their interaction as moderators
                + (1|Study.ID) #group level effect #1
...
posterior.plot <- posterior_fit %>% ggplot()+
  geom_density_ridges(aes(x=value, y = Environment, fill = Sex), alpha = 0.4, scale = 0.6, position = position_nudge(y = 0.15), height = 10, show.legend = F)+
  geom_vline(xintercept = 0, linetype = 2, colour = "black") + 
    ylab("Environment")+
  xlab("\nEffect Size (Hedges' g)")+
  scale_fill_brewer(palette = "Set1")+
...
# svg("figures/both_plot.svg", width=5.5, height=5.5)
# both.plots
# dev.off()
# 
# pdf("figures/both_plot.pdf",  width = 5.5, height = 5.5)
# both.plots
# dev.off()
```
<br></br>
