...
brms.preliminary <- brm(g | se(SE)  ~ 1 + Sex * Environment + log(Generations) + Blinding 
...
bayesplot::mcmc_areas(posterior_samples(brms.preliminary)[,1:11]) + geom_vline(xintercept = 0, linetype = 2)

new.data <- prelim.data %>% 
  mutate(SE = sqrt(var.g)) %>% 
  select(Sex, Environment, Blinding, Generations, SE) %>% 
  mutate(Generations = mean(log(Generations)), Blinding = "Blind", SE = 0) %>% 
  distinct() %>% arrange(Sex, Environment)
predictions <- data.frame(new.data, predict(brms.preliminary, newdata = new.data, re_formula = NA))

pd <- position_dodge(0.2)
predictions %>% ggplot(aes(Sex, Estimate, group = Environment)) + 
  geom_errorbar(aes(ymin = X2.5.ile, ymax = X97.5.ile), position = pd, width = 0) +
...
brms.complete <- brm(g | se(SE)  ~ 1 + Sex * Environment + Taxon + Outcome
                + (1|Study.ID), #group level effect #1
                family = "gaussian", 
                seed = 1,
                cores = 4, chains = 4, iter = 4000, # Run 4 chains in parallel for 4000 iterations (2000 are burn in)
...
brms.complete2 <- brm(g | se(SE)  ~ 1 + Sex * Environment + log(Generations) + Blinding + 
                + (1|Taxon) #group level effect #1
                + (1|Outcome) #group level effect #1
                + (1|Study.ID), #group level effect #1
                family = "gaussian", 
                seed = 1,
                cores = 4, chains = 4, iter = 4000, # Run 4 chains in parallel for 4000 iterations (2000 are burn in)
...
bayesplot::mcmc_areas(posterior_samples(brms.complete)[,1:24]) + geom_vline(xintercept = 0, linetype = 2)
bayesplot::mcmc_areas(posterior_samples(brms.complete2) %>% select(-starts_with("r"), -lp__)) + geom_vline(xintercept = 0, linetype = 2)

post <- (posterior_samples(brms.complete, 
                           pars = c("b_Intercept", "b_SexB", "b_SexF", 
                                    "b_EnvironmentStressed", "b_SexB:EnvironmentStressed", 
                                    "b_SexF:EnvironmentStressed", "b_OutcomeReproductiveSuccess")) %>%
  mutate(both_benign = b_Intercept + b_SexB + b_OutcomeReproductiveSuccess,
         both_stressful = b_Intercept + b_SexB + b_EnvironmentStressed + `b_SexB:EnvironmentStressed` + b_OutcomeReproductiveSuccess,
         male_benign = b_Intercept + b_OutcomeReproductiveSuccess,
         male_stressful = b_Intercept + b_EnvironmentStressed + b_OutcomeReproductiveSuccess,
         female_benign = b_Intercept + b_SexF + b_OutcomeReproductiveSuccess,
         female_stressful = b_Intercept + b_SexF + b_EnvironmentStressed + `b_SexF:EnvironmentStressed` + b_OutcomeReproductiveSuccess))[,-(1:7)]

post2 <- (posterior_samples(brms.complete, 
                           pars = c("b_Intercept", "b_SexB", "b_SexF", 
                                    "b_EnvironmentStressed", "b_SexB:EnvironmentStressed", 
                                    "b_SexF:EnvironmentStressed")) %>%
  mutate(both_benign = b_Intercept + b_SexB,
         both_stressful = b_Intercept + b_SexB + `b_SexB:EnvironmentStressed`,
         male_benign = b_Intercept,
         male_stressful = b_Intercept + b_EnvironmentStressed ,
         female_benign = b_Intercept + b_SexF ,
         female_stressful = b_Intercept + b_SexF + `b_SexF:EnvironmentStressed` ))[,-(1:6)]

bayesplot::mcmc_areas(post) + geom_vline(xintercept = 0, linetype = 2)

new.data <- restricted.data %>% 
  mutate(SE = sqrt(var.g)) %>% 
  select(Sex, Environment, Blinding, Generations, SE, Taxon) %>% 
  mutate(Generations = mean(log(Generations)), 
         Blinding = "Blind", 
         Taxon = "Fly",
         Outcome = "Reproductive Success",
         SE = 0) %>% 
  distinct() %>% arrange(Sex, Environment)
predictions.complete <- data.frame(new.data, fitted(brms.complete, newdata = new.data, re_formula = NA))

pd <- position_dodge(0.2)
predictions.complete %>% ggplot(aes(Sex, Estimate, group = Environment, colour = Environment)) + 
  geom_errorbar(aes(ymin = X2.5.ile, ymax = X97.5.ile), position = pd, width = 0) +
  geom_point(position = pd) +
  coord_flip()
```
