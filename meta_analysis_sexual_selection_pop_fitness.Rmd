...
knitr::opts_knit$set(cache=TRUE, warning = F) # set up cacheing to save time when re-building the html document
```
...
p.meta <- full_dataset %>% 
  ...
  scale_x_continuous(limits=c(-3.35, 4.25), 
...
Individual Meta-analyses for each fitness related trait with n>3: 
...
Given that none of the levels of `Taxon` have a particularly strong impact on effect size, and many categories are incompletely sampled with regards to Sex and Environment (Table S8), we elected to treat `Taxon` as a random/group level effect in all subsequent models.
...
**Table S9** Using the ``anova.rma`` function we can conduct hypothesis tests between two categorical groups in the model. Here we conduct 5 tests comparing the relative effect of sexual selection between the sexes, and in different environments. 
...
As per reviewers comments, we can use the log-response ratio instead of hedges g. Here we use the ``metafor`` function escalc. 
...
summary(lnRR.env.sex.model, digits = 2)
...
```{r}
env.sex.mean.only <- rma.mv(g, V = var.g, 
                          mods = ~ 1 + Sex * Environment, 
                          random = list(~ 1 | Study.ID, 
                                       ~ 1 | Outcome,
                                       ~ 1 | Taxon), 
                          method = "REML", 
                          data = strict_dataset %>% filter(mean.low != "NA"))
summary(env.sex.mean.only, digits = 2)
```
...
Variability.data <- left_join(lnVR.data, lnCVR.data)

Variability.data %>% ggplot(aes(x = lnVR, y = lnCVR)) + geom_point()

cor(Variability.data$lnVR, Variability.data$lnCVR, use = "complete.obs")
```
...
if(!file.exists("data/lnVR.brms.rds")){
  lnVR.brms <- brm(lnVR| se(lnVR_var)  ~ 1 + Sex * Environment 
                + (1|Taxon) 
                + (1|Study.ID)
                + (1|Outcome),
                family = "gaussian", 
                ...
                cores = 4, chains = 4, iter = 4000, 
                control = list(adapt_delta = 0.999, max_treedepth = 15),
                data =  lnVR.data %>% filter(Outcome.Class != "Ambiguous" & Environment != "Not Stated"))
...
```
<br></br>
**Figure S5:** A funnel plot of 459 effect sizes shows asymmetry, indicating potential publication bias, egger's regression test for funnel plot asymmetry also suggests the plot is asymmetrical (z = 6.22, p < 0.0001). The asymmetry appears to come from a spread of positive effect sizes outside the funnel and of varying degrees of precision. Counter to expectations of publication bias these positive studies are not just 'low precision, large effect' results. Funnel plot asymmetry may also be due to genuine heterogeneity in effect sizes between studies, which is high in the present meta-analysis because it covers many species, outcome measurements, and experimental designs. 
...
JIF.plot <- ggplot(data = full_dataset, aes(x=JIF, y=g))+
  geom_jitter(color='darkgreen', alpha = 0.4, aes(size = 1 / var.g), show.legend = FALSE)+
...
```
<br></br>
**Figure S6:** Journal impact factor is not correlated with effect size. Point size is proportional to the precision of the effect size.
...
```
<br></br>
**Figure S7:** The effect size dataset shows little to no signs of the time-lag bias: the average effect size from published studies has remained consistent across the two previous decades. Point size is proportional to the precision of the effect size.
