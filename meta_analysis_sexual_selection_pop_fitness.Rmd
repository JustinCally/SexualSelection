...
fitness.component.predictions %>% cbind.data.frame(as.data.frame(BFs) %>% t() %>% as.data.frame() %>% `colnames<-`("BF")) %>% add_significance_stars2() %>% `row.names<-`(NULL) %>% pander(split.cell = 40, split.table = Inf, digits = 2)
```
