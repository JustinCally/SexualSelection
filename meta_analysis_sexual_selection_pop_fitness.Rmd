...
**Table S3:** Table of effect sizes included in our meta-analysis. See the text following the data table for an explanation of each column.
 <br/><br/> 
<input type=button class=hideshow></input>
```{r message=FALSE}
...
```
 <br/><br/> 
**Table S4:** Table of fitness outcomes included in our meta-analysis by sex.
 <br/><br/> 
<input type=button class=hideshow></input>
```{r}
Outcome_and_sex <- as.data.frame.matrix(table(prelim.data$Outcome, prelim.data$Sex))
colnames(Outcome_and_sex) <- cbind("Both", "Male", "Female")
...
p.meta <- prelim.data %>% 
  ...
  geom_point(size = 3) +  #Add data points and color them black
...
```
<br></br>
**Figure 1:** Forest plot of raw effect sizes and their 95% confidence intervals, grouped according to environment (stressed or unstressed) and the sex of the individuals whose fitness trait was measured (male, female, or both sexes mixed together). Rows with multiple data points denote studies that provided multiple effect sizes.
...
### Exploring our data with the large set of covariates
...
Taxon.metaanlysis <- predictions.complete %>% ggplot(aes(x = pred, 
                                                         y= Environment, 
                                                         fill = Sex)) + 
  geom_vline(xintercept = 0, linetype = 2, colour = "grey70") + 
...
This model indicates quite a bit of heterogeneity between taxon. However, more striking is that confidence bands increase for taxa with low sample size. As previously shown, the beetle and fly taxa are the most heavily sampled and in the above figure have the narrowest confidence bands. The overall direction of effect does not change, although guppies and mice show effect sizes ~ 0. We can see from this that under stressed environments, females from all taxa appear to have greater fitness increase than males or 'both'. 
...
EnvSex.metaanalysis <- predictions.complete2 %>% ggplot(aes(x = pred, y= Environment, fill = Sex)) + 
  ...
  geom_errorbarh(aes(xmin = predictions.complete2$ci.lb, 
                     ...
                     color = Sex), height = 0, position = pd) +
  geom_point(position = pd, size=3, shape=21, color = "grey20") + 
  ylab("Environment\n")+
...
Although not previously done extensively, it seems that the best way to conduct this analysis is not through subsetting but through utilising model predictions as we did with Hedges' g previously. This can be done be utilising the same predict function but for lnCVR and var.lnCVR. 

Multilevel-model using lnCVR:

```{r}
#Now try with multilevel model 
variance.model <- rma.mv(lnCVr, var.lnCVr, mods = ~ 1 + Sex + Environment + Sex:Environment, 
...
pd <- position_dodgev(height = .3)
var.plot <- predictions.variance %>% ggplot(aes(x = pred, y= Environment, fill = Sex)) + 
  geom_vline(xintercept = 0, linetype = 2, colour = "grey70") + 
...
(Promislow 1998 has one sample size of >10,000) and is not shown here. From these plots we can see that with increased sample size the effect sizes are closer to zero. This trend should be taken into account as meta-analytic models are wighted by 1/variance. 
...
prelim.data %>% ggplot(aes(x=Generations, y=g, fill=Taxon))+
  geom_point(shape=21, color = "grey20", size=2)+
  ylim(-3.5,3.5)+
  geom_hline(yintercept=0, linetype="dashed") + 
  scale_fill_brewer(palette = "Set3")+
  labs(y='Effect size (Hedges g)', x= 'Generations')
```
