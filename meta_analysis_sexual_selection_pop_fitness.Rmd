...
# run meta-analysis using the restricted data, and moderators
...
# I cut all these out, since the trime-and-fill doesn't work, and those sub-models probably don't make sense
...
make.funnel <- function(dataset, model){
  ...
  ggplot(dataset, aes(x = sqrt(var.g), y = g)) +
    ...
    geom_segment(aes(x = min(se.seq), y = meanll95, xend = max(se.seq), yend = meanll95), linetype='dotted', data=dfCI, colour = "red") +
    geom_segment(aes(x = min(se.seq), y = meanul95, xend = max(se.seq), yend = meanul95), linetype='dotted', data=dfCI, colour = "red") +
    scale_x_reverse() +
